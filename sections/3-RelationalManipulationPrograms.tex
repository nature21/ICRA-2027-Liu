...
\begin{table}[t]
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.37\columnwidth}
        ...
        \midrule
        Semantic goal $G_i$
            & Flip the target object against a fixture for a specific angle. \\
        \rowcolor{black!5}
        Relational interface $C_i$
            & Semantic roles $\mathbf{r}_i$: target object, support plane, fixture, and pusher.
              Relations $\Gamma_i$: the object rests on the support plane, and the fixture
        ...
        Geometry resolver $\rho_i$
            & At each rollout state, resolve the support plane, the vertical face of the fixture, the flipping axis, and dimensions of the target object. \\
...
\end{equation*}
Here, $\mathbf{r}_i$ declares the semantic roles involved in the primtive, $\mathbf{q}_i$ declares the effect parameter, and $\Gamma_i$ specifies the relations among the roles. For example, for the primitive \textit{flip} in Table~\ref{tab:flip_primitive}, $\mathbf{r}_i$ declares the relevant semantic roles \textit{(target object, support plane, fixture, pusher)}, and the effect parameter $\mathbf{q}_i$ is the requested flipping angle. To apply a primitive in a scene, an invocation supplies runtime arguments $(b_i,g_i)$ satisfying
\begin{equation*}
    b_i\in\operatorname{Bind}_E(\mathbf{r}_i,\Gamma_i),
    \qquad
    g_i\in\operatorname{Val}(\mathbf{q}_i).
\end{equation*}
Here, $\operatorname{Bind}_E(\mathbf{r}_i,\Gamma_i)$ denotes the type-compatible assignments of the objects in the scene $E$ to $\mathbf{r}_i$ that satisfy $\Gamma_i$, and $\operatorname{Val}(\mathbf{q}_i)$ denotes the values matching the types declared by $\mathbf{q}_i$. Scene-dependent geometry remains internal to the primitive and is resolved by $\rho_i$ at execution time.
...
\end{equation*}
We solve this trajectory-optimization problem with the Cross-Entropy Method (CEM)~\cite{deboer2005tutorial}, a sampling-based optimizer, which is gradient-free and allows us to optimize robot motions without differentiating through the contact-rich simulation. The primitive thereby adapts its motion to the object pose, size, geometry, physical properties, and requested effect.
